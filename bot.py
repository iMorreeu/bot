...
@tree.command(name = "get-lol", description = "Generate an account.", guild=discord.Object(id=config["guild-id"]))
...
async def gen(interaction: discord.Interaction, account: str):
    ...
    for file in glob.glob(directory + '/accounts/*.txt'):
        ...
        if file_name.lower() == account.lower():
            ...
            return
    await simpleEmbed(interaction, config["messages"]["category-doesnt-exist"], True)
...
@tree.command(name = "create", description = "Creates a new category. (ADMIN ONLY)", guild=discord.Object(id=config["guild-id"]))
...
async def create(interaction: discord.Interaction, premium: bool, name: str):
    if premium:
        ...
        await interaction.response.send_message("Created new category: ``" + name.lower() + "``")
...
@tree.command(name = "add", description = "Adds an account to a category. (ADMIN ONLY)", guild=discord.Object(id=config["guild-id"]))
@app_commands.checks.has_permissions(administrator=True)
async def addacc(interaction: discord.Interaction, premium: bool, category: str, account: str):
    if premium:
        for file in glob.glob(directory + '/paccounts/*.txt'):
            if getFileName(file).lower() == category.lower():
